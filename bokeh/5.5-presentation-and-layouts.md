...
You've seen some examples in last sessions. We can nest as many rows, columns, or grids of plots together as we like.
...
The `gridplot()` function can be used to arrange Bokeh Plots in grid layout. It also collects all tools into a single toolbar, and the currently active tool is the same for all plots in the grid. It is possible to leave “empty” spaces in the grid by passing `None` instead of a plot object.
